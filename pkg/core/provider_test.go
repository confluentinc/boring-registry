...
func TestSha256Checksum(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			// NOTE: checksum of the in-memory zip, which depends on the archive/zip
			// serialization. Go 1.27's archive/zip changed the emitted bytes, so this
			// expected value was regenerated under the 1.27 toolchain.
			expected: []byte{0xd7, 0xfc, 0xcc, 0xec, 0x85, 0xc7, 0x2a, 0x1, 0x96, 0xab, 0x99, 0x59, 0xb3, 0x18, 0x57, 0x49, 0xa0, 0x9d, 0x8a, 0x62, 0x89, 0x69, 0xb7, 0x34, 0xe3, 0x18, 0x12, 0xdc, 0x70, 0xb9, 0xb, 0xcf},
		},
	}
